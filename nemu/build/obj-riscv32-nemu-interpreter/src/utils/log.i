...
# 27 "src/utils/log.c"
  ...
  do { printf("\33[1;34m" "[%s:%d %s] " "Log is written to %s" "\33[0m" "\n", "src/utils/log.c", 30, __func__, log_file ? log_file : "stdout"); do { extern FILE* log_fp; extern 
# 30 "src/utils/log.c" 3 4
 _Bool 
# 30 "src/utils/log.c"
 log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Log is written to %s" "\33[0m" "\n", "src/utils/log.c", 30, __func__, log_file ? log_file : "stdout"); fflush(log_fp); } } while (0); } while (0);
}


# 33 "src/utils/log.c" 3 4
_Bool 
# 33 "src/utils/log.c"
    log_enable() {
  return 
