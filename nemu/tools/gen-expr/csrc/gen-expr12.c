...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < loop; i ++) {
    ...
    FILE *testbench12 = fopen("/home/jinghanhui/ysyx-workbench/nemu/tools/gen-expr/results/testbench12", "a");//创建文件
    FILE *fp = fopen("/tmp/.code12.c", "w");//创建文件
    assert(fp != NULL);
    fputs(code_buf, fp);//写入答案计算代码
    fclose(fp);//关闭文件

    int ret = system("gcc -Werror /tmp/.code12.c -o /tmp/.expr12  > /dev/null 2>&1");//看有无除0有就滤除该表达式
    if (ret != 0) continue;

    fp = popen("/tmp/.expr12", "r");//返回代码计算结果,管道
    assert(fp != NULL);

    uint32_t result;
    ret = fscanf(fp, "%u", &result);//读取结果存到ret
	 fprintf(testbench12,"%u %s\n", result, buf);//打印表达式以及电脑计算的值,写到文件里面去
    pclose(fp);
    fclose(testbench12);
   }
  return 0;
